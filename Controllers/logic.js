...
facultyRegister = async (req, res) => {
    ...
    //LRS4 for erpdetails
    const { rollNo, fname, lname, email, password, mobile, gender, department, location } = req.body
    if (!rollNo || !fname || !lname || !email || !password || !file || !mobile || !gender || !department || !location) {
        res.status(404).json("All inputs are required")
    }
    try {
        const preFaculty = await facultys.findOne({ email })

        if (preFaculty) {
            res.status(403).json("Faculty Already Present")
        }
        else {
            // create object for new student
            const newFaculty = new facultys({ rollNo, fname, lname, email, password, mobile, gender, department, profile: file, location })
            await newFaculty.save()

            res.status(206).json(newFaculty)
        }
    }
    catch (err) {
        res.status(400).json("Logic Error")
...
getAllFacultys = async (req, res) => {
    ...
    try {
        const allFacultys = await facultys.find(query)
        res.status(207).json(allFacultys)
    }
    catch (err) {
        res.status(400).json(err)
    }
...
// GSES3 get single employyees after exports goto router
getSingleFaculty = async (req, res) => {
    const param = req.params.id//or const {id}=req.params.id
    try {
        const sFaculty = await facultys.findOne({ _id: param })
        res.status(200).json(sFaculty)
    }
    catch (err) {
        res.status(400).json(err)
    }
...
// logic to remove faculty
removeFaculty = async (req, res) => {
    const param = req.params.id
    try {
        const removedFaculty = await facultys.findByIdAndDelete({ _id: param })
        res.status(200).json(removedFaculty)
    }
    catch (err) {
        res.status(400).json(err)
    }
...
editFaculty = async (req, res) => {
    const param = req.params.id
    const { rollNo, fname, lname, email, password, mobile, gender, department, location, faculty_profile } = req.body
    const file = req.file ? req.file.filename : faculty_profile
    if (!rollNo || !fname || !lname || !email || !password || !mobile || !gender || !department || !location) {
        res.status(404).json("All inputs are required")
    }
    try {
        const user = await facultys.findOne({ _id: param })
        if (user) {
            user.rollNo = rollNo
            user.fname = fname
            user.lname = lname
...
facultyLogin = async (req, res) => {
    ...
    try {
        ...
        const user = await facultys.findOne({ email })
        if (!user) {
            return res.status(404).json({
                message: 'Used not found',
                success: false
